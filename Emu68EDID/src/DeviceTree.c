...
APTR GetPropValue(STRPTR keyName, STRPTR propertyName, ULONG * length)
{
	APTR key, property, value = NULL;
	
	if (key = DT_OpenKey(keyName))
	{
		if (property = DT_FindProperty(key, propertyName))
		{
			if (length != NULL)
			{
				*length = DT_GetPropLen(property);
			}
			
			value = (APTR)DT_GetPropValue(property);
		}
	}
